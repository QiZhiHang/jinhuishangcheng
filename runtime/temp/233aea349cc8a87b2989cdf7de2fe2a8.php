<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:42:"./template/mobile/new/user\order_list.html";i:1502075918;s:42:"./template/mobile/new/public\header11.html";i:1502075948;s:38:"./template/mobile/new/public\menu.html";i:1490353004;}*/ ?>
<!DOCTYPE html >
<html>
<head>
...
<form action="<?php echo U('Mobile/order_list/ajax_order_list'); ?>" name="filter_form" id="filter_form">  
      <div class="Evaluation2">
            <ul>
              <li><a href="<?php echo U('/Mobile/User/order_list'); ?>" class="tab_head <?php if(\think\Request::instance()->param('type') == ''): ?>on<?php endif; ?>"  >全部</a></li>
              <li><a href="<?php echo U('/Mobile/User/order_list',array('type'=>'WAITPAY')); ?>"      class="tab_head <?php if(\think\Request::instance()->param('type') == 'WAITPAY'): ?>on<?php endif; ?>">待付款</a></li>
              <li><a href="<?php echo U('/Mobile/User/order_list',array('type'=>'WAITSEND')); ?>"     class="tab_head <?php if(\think\Request::instance()->param('type') == 'WAITSEND'): ?>on<?php endif; ?>">待发货</a></li>
              <li><a href="<?php echo U('/Mobile/User/order_list',array('type'=>'WAITRECEIVE')); ?>"  class="tab_head <?php if(\think\Request::instance()->param('type') == 'WAITRECEIVE'): ?>on<?php endif; ?>">待收货</a></li>
              <li><a href="<?php echo U('/Mobile/User/order_list',array('type'=>'WAITCCOMMENT')); ?>" class="tab_head <?php if(\think\Request::instance()->param('type') == 'WAITCCOMMENT'): ?>on<?php endif; ?>">待评价</a></li>
            </ul>
      </div>     
       ...
       <div class="order_list">
          <h2>
              <a href="javascript:void(0);">
                  <img src="__STATIC__/images/dianpu.png"><span>订单号:<?php echo $list['order_sn']; ?></span>
				  ...
				  <dd class="pice" style="width:80%">                  
                  	<em>
                        <?php if(($list[return_btn] == 1) and ($good[is_send] < 2)): ?>
                          	<a href="<?php echo U('Mobile/User/return_goods',array('order_id'=>$list[order_id],'order_sn'=>$list[order_sn],'goods_id'=>$good[goods_id],'spec_key'=>$good['spec_key'])); ?>" style="color:#999;">申请售后</a>
                          <?php endif; ?>
                    </em>
                  </dd>                  
          ...
          <div class="anniu" style="width:95%">
                <?php if($list['cancel_btn'] == 1): ?><span onClick="cancel_order(<?php echo $list['order_id']; ?>)">取消订单</span><?php endif; if($list['pay_btn'] == 1): ?><a href="<?php echo U('Mobile/Cart/cart4',array('order_id'=>$list['order_id'])); ?>">立即付款</a><?php endif; if($list['receive_btn'] == 1): ?><a href="<?php echo U('Mobile/User/order_confirm',array('id'=>$list['order_id'])); ?>">收货确认</a><?php endif; if($list['comment_btn'] == 1): ?><a href="<?php echo U('/Mobile/User/comment'); ?>">评价</a><?php endif; if($list['shipping_btn'] == 1): ?><a href="<?php echo U('User/express',array('order_id'=>$list['order_id'])); ?>">查看物流</a><?php endif; ?>
          </div>
       </div>
		<?php endforeach; endif; else: echo "" ;endif; ?>  
    </div>
