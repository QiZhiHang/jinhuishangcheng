...
//        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    $update_data = array(
        ...
        'my_fanxian_money'   => ['exp','my_fanxian_money+'.$my_fanxian_money],
    );
    
	if(($user_money+$pay_points+$distribut_money+$my_fanxian_money) == 0)
...
function fanxianLog($user_id, $user_money = 0,$desc = '')
{
    $account_log = array(
        'user_id'       => $user_id,
...
//        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    $update_data = array(
        'now_money'        => ['exp','now_money+'.$user_money],
...
function update_pay_status($order_sn,$ext=array())
{
    ...
    function confirm_order($id,$user_id = 0){
       ...
       // var_dump($order_goods);die;
        if(!empty($order_goods)){
            ...
            if ($user_info) {
                //对自己进行返利处理
                fanxian_money($order_goods,$user_info);
                //对商品进行三级返利
                sanji($order_goods,$user_info);
            }
 ...
 function fanxian_money($order_goods,$user_info){
       ...
       // $data['is_tixian'] = 0;
       // // $data['now_money'] = 0;
       // $res = M('users')->where('user_id='.$data['user_id'])->save($data);
       accountLog($data['user_id'],0,0,'购买特殊商品返现'.$data['my_fanxian_money'],0,$data['my_fanxian_money']);
      return $res;
    }

    /*三级分销*/

 function sanji($order_goods,$user_info){
   ...
   function getMemberinfo($user_id,$field){
        //$where = 
