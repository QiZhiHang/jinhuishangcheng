...
class User extends MobileBase
{
    ...
    */
    ...
    public function dalesong(){

        /*$this->assign('today_num', '0.'.tpCache('basic.today_num'));
        $this->assign('user_money', intval($this->user['my_fanxian_money']));
        $this->assign('this_point', intval($this->user['user_money']/10000) * tpCache('basic.today_return_rate'));
        $this->assign('user_points',intval($this->user['pay_points']));*/
        //$data = M('fanxian_money')->where(['status'=>0,'user_id'=>$this->user['user_id']])->find();
            $this->assign('every_day_money',($this->user['my_fanxian_money']*0.2)/10);
            $this->assign('now_money',$this->user['pay_points']);
            $this->assign('user_money', intval($this->user['my_fanxian_money']));
            return $this->fetch();
    }
    /*
    ...
    {   
        ...
        $this->assign('coupon_count', $this->user['my_fanxian_money']);
...
//            header("Location: " . U('User/Index'));
    ...
    public function order_confirm()
    {
        ...
        if (!$data['status']) {
           ...
           /* if(!empty($order_goods)){
                ...
                //对自己进行返利处理
                $this->fanxian_money($order_goods);
                //对商品进行三级返利
                $this->sanji($order_goods);
    ...
    private function fanxian_money($order_goods){
       ...
       accountLog($user_id,0,0,'购买特殊商品返现'.$data['my_fanxian_money'].'元',0,$data['my_fanxian_money']);
      return $res;
    }

    /*三级分销*/

    private function sanji($order_goods){
            ...
            if(!empty($order_goods['result'])){
                ...
                if($teshu>0){
                    ...
                    $member_info = $this->getMemberinfo(intval($this->user_id),'*');
                    $fr_recommend_info = $member_info;
                        foreach ($array as $k => $v) {
                            $fr_recommend_info = $this->getMemberinfo($fr_recommend_info['user_recommend_id'],'*');

                            if(empty($fr_recommend_info)){
                                break;
                            }

                           accountLog($fr_recommend_info['user_id'],$teshu*($v/100),0,'下线成员购买商品返利金额',0,0); 
                ...
                if($yiban>0){
                            ...
                            $recommend_id = $this->user['user_recommend_id'];
                            $recommend_info = $this->getMemberinfo($recommend_id,'*');

                            if(empty($recommend_info)){
                                break;
                            }

                           accountLog($recommend_info['user_id'],$teshu*(5/100),0,'下线成员购买商品返利金额',0,0);
    ...
    public function getMemberinfo($user_id,$field){
        //$where = 
        $user_id = intval($user_id);
        return M('users')->where('user_id='.$user_id)->field($field)->find();
    }
...
//        $goods = M('goods')->where("goods_id", $goods_id)->find();
    ...
    public function recharge_add()
    {
        ...
        $user = session('user');
        $res = M('recharge')->where(array('user_id'=>$user['user_id'],'pay_status'=>0))->count();
        if ($res > 0) {
            $this->error("您有未完成的记录，请先处理");
        }
    ...
    public function withdrawals()
    {
        ...
        if (IS_POST) {
            ...
            if (I('post.withdrawals_type') == 1) {
                if ($data['money'] > $this->user['user_money']) {
                    $this->error("你最多可提现{$this->user['user_money']}账户余额.");
                    exit;
                }
                $last_re = M('recharge')->where(array('user_id'=>$this->user_id,'pay_time'=>array('gt',0)))->order('pay_time desc')->find();
                ...
                if($data['money'] > $this->user['my_fanxian_money']){
                    ...
                    $this->error("你最多可提现{$this->user['my_fanxian_money']}");
                }else{
